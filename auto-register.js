...
async function waitForVerificationCode(mailjs, maxAttempts = 20) {
    logger("等待验证码...");
    for (let i = 0; i < maxAttempts; i++) {
        try {
            const messages = await mailjs.getMessages(1);
            if (messages.status && messages.data.length > 0) {
                const latestMessage = await mailjs.getMessage(messages.data[0].id);
                const messageText = latestMessage.data.text;
            ...
            logger(`检查邮件时出错: ${err.message}`, "error");
        }
    }
        ...
        for (let i = 0; i < number; i++) {
            try {
                ...
                if (email && password) {
                    ...
                    const otp = await waitForVerificationCode(mailjs);
                    logger(`验证码为：${otp}`);
